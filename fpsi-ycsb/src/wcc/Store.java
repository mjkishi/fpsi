...
public class Store {
	...
	public ReadResponse getLatestObject1(RequestId reqId,String Id,int [] view, int[] hasRead, int numNodes, int isUpdate, int reader){		
		...
		if(isUpdate==1) {
			...
			/* for(int i=0; i<numNodes;i++){
			    System.out.println("RequestId "+reqId+" has vectorclock "+view[i]+" in position "+i+" and version is "+version);
			 }*/
		...
		///////////////////////////////////////////////////////////////////////////read-only //////////////////////////////////////////////////////////////////
			...
			for(int i=0; i<numNodes;i++){	
				...
				if(isUpdate == 0  && store.get(Id).getSitehistory(i).getMainRWInfo(Id, lastSeqNo).contains(reqId)) {
					seqNo=lastSeqNo-1;

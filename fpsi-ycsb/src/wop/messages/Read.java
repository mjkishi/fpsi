...
public final class Read extends Message {
	...
	public Read(DataInputStream input) throws IOException { 	
		super(input);//super
		sizeOfId=input.readInt();//4
		byte[] ObjIdbyte=new byte[sizeOfId];//length
		input.readFully(ObjIdbyte);
		this.ObjectId=new String(ObjIdbyte,"UTF-8") ; 
		numNodes=input.readInt();//4
		hasRead=new byte[numNodes*4];//4*
		input.readFully(hasRead);
		VC=new byte[numNodes*4];//4*
		input.readFully(VC);
		isUpdate=input.readInt();//4

 ...
 * process, provides two methods for establishing new connection: active and
...
public class TcpConnection {
	 ...
	 * @param replica - process to connect to.
	...
	private void connect() throws InterruptedException {
		if (active) {
			...
			// this is passive connection so we are waiting until other process
			...
			synchronized (this) {
				while (!connected) {
					...
					if(ProcessDescriptor.getInstance().getLocalProcess().getId()==ProcessDescriptor.getInstance().numReplicas-1 ) {
